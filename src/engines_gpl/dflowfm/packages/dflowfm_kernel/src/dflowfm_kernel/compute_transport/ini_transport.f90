...
!     only for 2D, implicit
      ...
      ! Always call alloc_transport: grid-sized arrays (ndeltasteps, jaupdatehorflux,
      ! jaupdate, dtmax, dxiAu) must be allocated even when NUMCONST=0, otherwise
      ! gfortran crashes on scalar broadcast to unallocated allocatable.
