...
subroutine find_n()
    ...
    ! Locals
    integer :: idis
    ! --- THE CORRECT TYPE AND RANK FOR THE MASK POINTER ---
    integer, pointer :: null_mask(:) => null()
    ! ------------------------------------------------------
    !
    ! Body
    call nearest_neighbour(nf_num_dif, x_diff  , y_diff  , null_mask, n_diff  , Huge(1.d0), fm_xzw, fm_yzw, fm_ndx, 0, 0)
    call nearest_neighbour(nf_num_dif, x_intake, y_intake, null_mask, n_intake, Huge(1.d0), fm_xzw, fm_yzw, fm_ndx, 0, 0)
    do idis = 1, nf_num_dif
        call nearest_neighbour(no_amb(idis), x_amb(idis,:), y_amb(idis,:), null_mask, n_amb(idis,:), Huge(1.d0), fm_xzw, fm_yzw, fm_ndx, 0, 0)
    enddo
